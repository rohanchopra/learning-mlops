...
def prepare_data(data, categorical):
...
def save_data(data, filename):
    ...
    if s3_endpoint:
        ...
        data.to_parquet(filename, engine='pyarrow', index=False, storage_options=options)
    else:
        data.to_parquet(filename, engine='pyarrow', index=False)
